...
exports.default = function (contract) {
  return contract.abi.map(function (method) {
    ...
    var outputs = method.outputs || []; // eslint-disable-line prefer-destructuring;
    try {
      ...
      if (typeof devDocs.return === 'string' && method.outputs.length === 1) {
        outputs = method.outputs.map(function (param) {
          return _extends({}, param, { description: devDocs.return });
        });
      } else {
    ...
    return _extends({}, method, devDocs, userDocs, {
      ...
      outputs: outputs,
      signature: signature,
      signatureHash: signature && (method.type === 'function' ? (0, _helpers.getFunctionSignature)(signature) : (0, _helpers.getEventTopic)(signature))
    });
  });
};
