...
function formatTable(argList) {
  ...
  if (columns.length > 0) {
    ...
    return '' + argList.map(function (obj) {
...
var template = function template(it) {
  return '\n# ' + it.name + '\n\n' + (it.author ? it.author + '\n\n' : '') + it.abiDocs.map(function (docItem) {
    return docItem.type === 'event' || docItem.type === 'function' ? '## *' + docItem.type + '* ' + docItem.name + '\n\n' + [it.name + '.' + docItem.name + '(' + docItem.argumentList + ')', docItem.anonymous ? '`anonymous`' : null, docItem.stateMutability ? '`' + docItem.stateMutability + '`' : null, docItem.signatureHash ? '`' + docItem.signatureHash + '`' : null].filter(function (v) {
      return v != null;
