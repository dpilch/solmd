...
var formatMethodWithoutName = function formatMethodWithoutName(docItem) {
  return '(' + docItem.inputs.map(function (_ref10) {
    var type = _ref10.type,
        indexed = _ref10.indexed,
        name = _ref10.name;
    return '*' + type + '*' + (indexed ? ' indexed' : '') + (name ? ' `' + name + '`' : '');
  }).join(', ') + ')';
};
...
var formatMethodAnchor = function formatMethodAnchor(docItem) {
  return formatMethod(docItem).toLowerCase().replace(/\(\)/g, '').replace(/\(/g, '-').replace(/ /g, '-').replace(/[^-\w]+/g, '');
};

var tableOfContents = exports.tableOfContents = function tableOfContents(it) {
  // const cNameLower = it.name.toLowerCase();
  var eventsDocs = it.abiDocs.filter(function (_ref11) {
  ...
  return '* [' + it.name + '](#' + it.name.toLowerCase() + ')\n' + (it.abiDocs.some(function (docItem) {
    ...
    return '    * [' + docItem.name + '](#' + formatMethodAnchor(docItem) + ')' + formatMethodWithoutName(docItem);
...
var constructorNote = function constructorNote(it) {
  var constructorDoc = it.abiDocs.find(function (_ref12) {
    var type = _ref12.type;
    return type === 'constructor';
  });

  return '- **Constructor**: ' + it.name + '(' + (constructorDoc ? constructorDoc.inputs.map(function (_ref13) {
...
var fallbackNote = function fallbackNote(it) {
  var fallbackDoc = it.abiDocs.find(function (_ref14) {
...
var eventsSection = function eventsSection(it) {
  var eventsDocs = it.abiDocs.filter(function (_ref15) {
