...
function formatTable(argList) {
  var columns = [['type', function (t) {
    return '*' + t + '*';
  }], ['name'], ['description']].filter(function (_ref3) {
...
var template = exports.template = function template(it) {
  return '# ' + it.name + '\n\n' + (it.title ? '### ' + it.title + '\n\n' : '') + (it.author ? '**Author**: ' + it.author + '\n\n' : '') + constructorNote(it) + fallbackNote(it) + eventsSection(it) + accessorsSection(it) + functionsSection(it) + '\n';
};